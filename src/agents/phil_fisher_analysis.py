"""Pure analyzer functions for Phil Fisher's investment framework.

Extracted from `phil_fisher.py` so they can be imported without pulling in
LangChain / LangGraph. The original agent module continues to own the LLM
glue; these pure functions are shared with the MCP server in `mcp_server/`.
"""

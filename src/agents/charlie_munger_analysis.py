"""Pure analyzer functions for Charlie Munger's investment framework.

Extracted from `charlie_munger.py` so they can be imported without pulling in
LangChain / LangGraph. The original agent module still contains the v1 LangGraph
glue (`charlie_munger_agent`, `generate_munger_output`) that wires these into
the AgentState pipeline. The MCP server in `mcp_server/` imports from here
directly so the Claude Code native layer can run Munger's scoring without any
LLM-framework dependency.

Scoring weights and thresholds are preserved exactly as written in v1 — do not
"clean them up." Munger's high bar (total_score >= 7.5 for bullish, <= 5.5 for
bearish) and the quality-dominated weighting (0.35 moat + 0.25 mgmt + 0.25
predictability + 0.15 valuation) are load-bearing.
"""

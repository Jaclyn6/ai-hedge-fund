"""Pure analyzer functions for Bill Ackman's investment framework.

Extracted from `bill_ackman.py` so they can be imported without pulling in
LangChain / LangGraph. The original agent module still uses these scoring rules
verbatim via its own code path; the MCP server in `mcp_server/` imports from
here directly so the Claude Code native layer shares the same logic.
"""

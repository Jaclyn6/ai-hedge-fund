"""Pure analyzer functions for Michael Burry's deep-value, contrarian framework.

Extracted from `michael_burry.py` so they can be imported without pulling in
LangChain / LangGraph. The original agent module still contains the LangGraph
glue (state, LLM calls, HumanMessage wiring); this module holds only the
numeric scoring rules so both the v1 LangGraph pipeline and the Claude Code
native MCP server can share the same source of truth.
"""
...
def derive_preliminary_signal(total_score: float, max_score: float) -> str:
    """Aggregate score → bullish / bearish / neutral using Burry's thresholds."""
    if max_score <= 0:
        return "neutral"
    if total_score >= 0.7 * max_score:
        return "bullish"
    if total_score <= 0.3 * max_score:
        return "bearish"
    return "neutral"
